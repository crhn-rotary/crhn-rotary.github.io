...
export const peachesCanOrder: Record<string, string> = {
  'pages.peaches.fundraiser.intro': `## Annual Peach Sale and Fundraiser
**{pounds} pound (lb) box for \${dollars}**
...
We work directly with orchards, so please be sure to order by {orderByDate} because we are not able to make deadline exceptions.
...
Peaches are picked fresh, then delivered to us for the day of pickup. Pickup will be on {pickupDate}.`,
  ...
  'pages.peaches.fundraiser.ordering':
    'We work directly with orchards, so please be sure to order by {date} because we are not able to make deadline exceptions.',
...
export const text: Record<string, Record<string, string>> = {
  afterPickup: {
    ...
    ...peachesPageText,
    'pages.peaches.fundraiser.intro': `## Annual Peach Sale and Fundraiser
**For {lastYear}, we were able to offer an {pounds} pound (lb) box for \${dollars}.**

Our annual peach fundraiser will begin again in {monthYear}. Last year, we were able to order peaches from Colorado's Western Slopes, from Palisade. We will update this site when the fundraiser begins and let you know where we will be getting them from this year!

The peaches are freestone, about baseball size, and are sold by the box.

---

Check back in {monthYear} for what we can offer next year!
...
We work directly with orchards and they imposed a deadline of {orderByDate} for ordering. Unfortunately this deadline has passed, so ordering is now closed.
